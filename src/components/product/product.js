...
{/*<span className="shap inline-flex"></span> */}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <div className="group overflow-hidden relative" key={u._id}>
 ...
 <img src={`${process.env.NEXT_PUBLIC_API_URL}/uploads/${u.mainImage}`} alt="Product-1" className="w-full h-full object-cover object-top hover:scale-110 transition-all duration-700" />
          ...
          <div className="p-4 relative">
            ...
            <div className="z-20 relative bg-white">
             <Link href={`/product/${u._id}`}><h6 className="text-[15px] font-semibold text-slate-900 truncate">{u.title}</h6></Link>
              <h6 className="text-sm text-slate-600 font-medium mt-2">₹{u.price}</h6>
            </div>
          </div>
        </div>
))}
